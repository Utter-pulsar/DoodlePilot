...
interface DoodleBoxProps {
  ...
  fill?: string
  /** Rough fill pattern; 'cross-hatch' is denser/more opaque, 'solid' fully fills */
...
/**
 * A container whose border is drawn by Rough.js (the same engine Excalidraw uses),
 * so it looks genuinely hand-sketched. A FIXED seed keeps the sketch stable, and the
 * SVG scales (viewBox + preserveAspectRatio="none") while resizing — so the hand-drawn
 * lines don't re-roughen / dance every frame; we only redraw crisp once it settles.
 */
export function DoodleBox({
  ...
  useEffect(() => {
    ...
    draw()
    // During a resize the SVG just stretches (no re-roughen → no dancing lines);
    // redraw crisp only after it stops changing.
